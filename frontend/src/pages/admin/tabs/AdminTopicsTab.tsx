...
export default function AdminTopicsTab() {
  ...
  const fetch = useCallback(() => {
    ...
    adminApi.listTopics({ page, pageSize, search })
      .then(d => { setItems(d.topics || d.items || []); setTotal(d.total || 0); })
  ...
  const openEdit = (item: any) => { setDialog(item); setForm({ name: item.name || '', type: item.type || '', description: item.description || '' }); };
  const handleSave = async () => {
    setSaving(true);
    try { if (dialog === 'create') await adminApi.createTopic(form); else await adminApi.updateTopic(dialog.uid || dialog.id, form); setDialog(null); fetch(); } catch { /* ignore */ } finally { setSaving(false); }
  };
  const handleDelete = async () => { if (!deleteId) return; try { await adminApi.deleteTopic(deleteId); setDeleteId(null); fetch(); } catch { /* ignore */ } };

  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h6">Chủ đề</Typography>
        <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={openCreate}>Thêm chủ đề</Button>
      </Box>
      <Card sx={{ mb: 2 }}>
      ...
      {error ? <ErrorState title="Lỗi" message={error} onRetry={fetch} /> :
       ...
       items.length === 0 ? <Typography variant="body1" color="text.secondary" sx={{ textAlign: 'center', py: 4 }}>Không tìm thấy chủ đề</Typography> :
       <>
        <TableContainer component={Card}>
          <Table size="small">
            <TableHead><TableRow><TableCell>Tên</TableCell><TableCell>Loại</TableCell><TableCell>Mô tả</TableCell><TableCell align="right">Thao tác</TableCell></TableRow></TableHead>
      ...
      <Dialog open={Boolean(dialog)} onClose={() => setDialog(null)} maxWidth="sm" fullWidth>
        <DialogTitle>{dialog === 'create' ? 'Thêm chủ đề mới' : 'Chỉnh sửa chủ đề'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            <TextField fullWidth size="small" label="Tên" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} />
      ...
      <Dialog open={Boolean(deleteId)} onClose={() => setDeleteId(null)} maxWidth="xs" fullWidth>
        <DialogTitle>Xác nhận xóa</DialogTitle><DialogContent><Typography>Bạn có chắc chắn muốn xóa chủ đề này?</Typography></DialogContent>
