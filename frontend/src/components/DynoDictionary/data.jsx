...
function DynoDictionaryData({ isTOEIC }) {
  const { topics } = useTopics('vocab');
  const [toeicKey, setToeicKey] = useState(null);
  ...
  useEffect(() => {
    if (isTOEIC && topics.length > 0) {
      const toeic = topics.find(t => t.title?.toLowerCase().includes('toeic'));
      if (toeic) setToeicKey(toeic.key || toeic._id);
    }
  }, [isTOEIC, topics]);

  useEffect(() => {
    if (isTOEIC && toeicKey && packInfo.topics.length === 0) {
      setPackInfo(prev => ({ ...prev, topics: [toeicKey] }));
    }
  }, [isTOEIC, toeicKey]);
