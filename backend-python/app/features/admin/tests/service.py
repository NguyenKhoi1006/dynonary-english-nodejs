...
async def list_tests(
    ...
    level: str = "", type_filter: str = "",
    published: str = "", search: str = "",
...
async def get_test(test_id: str) -> Optional[Dict[str, Any]]:
    db = get_firestore_db()
    doc = db.collection(TESTS_COL).document(test_id).get()
    if not doc.exists:
        return None
    data = serialize_doc(doc.to_dict())
    data["id"] = doc.id
...
async def create_test(data: Dict[str, Any], admin_uid: str) -> Dict[str, Any]:
    db = get_firestore_db()
    doc_ref = db.collection(TESTS_COL).document()
...
async def update_test(test_id: str, data: Dict[str, Any], admin_uid: str) -> Optional[Dict[str, Any]]:
    db = get_firestore_db()
    doc_ref = db.collection(TESTS_COL).document(test_id)
